...
Future<Stream<Order>> getOrders({int limit = 6, int offset = 0, int deliveryType}) async {
  ...
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}orders?${_apiToken}with=user;foodOrders;foodOrders.food;foodOrders.extras;orderStatus;payment&search=user.id:${_user.id}&searchFields=user.id:=&orderBy=id&sortedBy=desc&delivery_dinein=${deliveryType}&limit=${limit}&offset=${offset}';

  final client = new http.Client();

  final request = http.Request('get', Uri.parse(url));

   print('Request URL orders: ${request.url}');
  // print('Request Headers orders: ${request.headers}');
