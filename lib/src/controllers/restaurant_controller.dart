...
import '../models/remove_from_favourite.dart';
...
class RestaurantController extends ControllerMVC {
  ...
  void listenForGalleries(String idRestaurant) async {
    ...
    //  print(_gallery.image);
      setState(() => galleries.add(_gallery));
    }, onError: (a) {}, onDone: () {});
  }

  void listenForFoodImages(String image) async {
    final Stream<Gallery> stream = await getGalleries(image);
    stream.listen((Gallery _gallery) {
      setState(() => galleries.add(_gallery));
    }, onError: (a) {}, onDone: () {});
  }
  ...
  void listenForIndividualFoods(String idRestaurant) async {
    final List<Food> foodList = await getIndividualFoodsOfRestaurant(idRestaurant);

    for (var food in foodList) {
      // print("DS>>> Why am i not called??? ");
      // print("DS>>> individual"+food.name);
      // Print each food item
      setState(() => foods.add(food));
    }

    /* if (foodList.isNotEmpty) {
      restaurant.name = foodList[0].restaurant.name;
    }*/
  }
 ...
 //////////////////////////////////////////////////////
  ...
  Future<AddToFavouriteModel> addRestaurantToFavouriteList(String restaurantId, String userId, String apiToken) async {
    final apiResponse = await addRestaurantToFavourite(restaurantId, userId, apiToken);
    // print("DS>> apiresponse "+ apiResponse.message);
    return apiResponse;
  }

  Future<DeleteFromFavouriteModel> removeRestaurantFromFavouriteList(String restaurantId, String apiToken) async {
    final apiResponse = await removeRestaurantFromFavourite(restaurantId, apiToken);
    // print("DS>> apiresponse "+ apiResponse.message);
    return apiResponse;
  }
